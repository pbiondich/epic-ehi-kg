# V_EHI_REG_ITEM_AUDIT_OYO

> Patient communication preferences audit events.This view contains audit information for patient communication preference records for the purpose of Electronic Health Information (EHI) export. If not in the EHI context, REG_ITEM_AUDIT_OYO and REG_ITEM_AUDIT_LINES_OYO should be used instead.
...
| 5 | `CHANGED_DATA_ELEMENT` | VARCHAR |  | The name of the Clarity table and column corresponding to the changed data. |
| 6 | `CHANGED_ITEM_NAME` | VARCHAR |  | Name of the item that was changed in the audit event. |
...
| 9 | `PREFERENCES_ID` | NUMERIC | FK→ | Communication preference that was changed in this audit event. |
