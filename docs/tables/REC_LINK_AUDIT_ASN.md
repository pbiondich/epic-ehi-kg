...
| 3 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple audit trail appointment serial numbers that are associated with the recommendation and the workflow from the REC_LINK_AUDIT_TRL table. |
| 4 | `AUD_APPT_SERIAL_NO` | NUMERIC |  | The appointment serial number for the recommendation audit trail entry. |
