...
> This table holds test dates for the hematocrit, hemoglobin, or other tests associated with the service line. The LN_TEST_DT_QUAL and LN_TEST_DT tables are related to one another. The number of lines in these tables will always be the same and values on the same line in each table will correspond to one another.
...
| 4 | `LN_TEST_DT` | DATETIME |  | This item holds the date of the last hematocrit, hemoglobin, or other test for the service line. |
