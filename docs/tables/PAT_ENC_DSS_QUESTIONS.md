# PAT_ENC_DSS_QUESTIONS

> The table PAT_ENC_DSS_QUESTIONS extracts the question contact serial numbers (LQL CSNs) for which an answer or comment is sent to decision support system to receive a response. This number is unique across all questions in your system. If you use IntraConnect, this is the Unique Contact Identifier (UCI).
...
| 3 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple question contact serial numbers associated with a patient order from the table PAT_ENC_DSS. |
