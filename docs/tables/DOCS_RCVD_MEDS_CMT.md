...
| 3 | `GROUP_LINE` | INTEGER | PK | The line number of the medication comment in the Received Document. Together with DOCUMENT_ID and CONTACT_DATE_REAL, this forms the foreign key to the DOCS_RCVD_MEDS table. |
| 4 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple medication comments associated with the received document and the medication from the DOCS_RCVD_MEDS table. |
| 5 | `CONTACT_DATE` | DATETIME |  | The date of this contact in calendar format. |
| 6 | `MED_COMMENTS` | VARCHAR |  | This item stores medication comments. |
