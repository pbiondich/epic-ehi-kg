...
> This table holds additional IDs used to identify the assistant dental surgeon for the service line to the other payor. The LN_ASST_SEC_QUAL, LN_ASST_SEC_ID, and LN_ASST_SEC_PYR tables are related to one another. The number of lines in these tables will always be the same and values on the same line in each table will correspond to one another.
...
| 4 | `LN_ASST_SEC_ID` | VARCHAR |  | This item holds additional IDs for the assistant dental surgeon. |
