...
> This table holds question identifiers (letters or numbers) for supporting documentation associated with the service line. The LN_DOC_QUESN_NUM, LN_DOC_QUESN_TYP, and LN_DOC_QUESN_RESP tables are related to one another. The number of lines in these tables will always be the same and values on the same line in each table will correspond to one another.
...
| 4 | `LN_DOC_QUESN_NUM` | VARCHAR |  | This item holds the question identifiers (numbers or letters) for the document. |
