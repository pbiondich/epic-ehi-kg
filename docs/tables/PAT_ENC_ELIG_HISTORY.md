...
| 7 | `ELIG_ACTION_C_NAME` | VARCHAR |  | This item holds the action performed on the patient's eligibility. |
| 8 | `ELIG_PLAN_INDEX` | INTEGER |  | This item indicates which eligibility plan the action was taken on. It is an index into the patient (EPT) 42010 group. |
| 9 | `ELIG_HX_USER_ID` | VARCHAR |  | This item holds the user who performed the action on the patient's eligibility information. |
| 10 | `ELIG_HX_USER_ID_NAME` | VARCHAR |  | The name of the user record. This name may be hidden. |
| 11 | `ELIG_HX_INST_UTC_DTTM` | DATETIME (UTC) |  | This item holds the instant that the eligibility action was performed. |
