...
> This table holds identifiers that identify the payors to which the ordering provider IDs apply. The LN_ORD_SEC_QUAL, LN_ORD_SEC_ID, and LN_ORD_SEC_PYR tables are related to one another. The number of lines in these tables will always be the same and values on the same line in each table will correspond to one another.
...
| 4 | `LN_ORD_SEC_PYR` | VARCHAR |  | This item holds the other payor identifier for the secondary ID on the line. The values in this item match the values in the OTH_PYR_ID column of the OTH_CVG_INFO table. |
