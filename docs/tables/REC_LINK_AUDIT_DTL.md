...
| 3 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple audit trail item details that are associated with the recommendation and the workflow from the REC_LINK_AUDIT_TRL table. |
| 4 | `AUDIT_ITEM_DETAIL` | VARCHAR |  | The detail that describes the appointment or order that was linked or unlinked in the recommendation audit trail entry. |
