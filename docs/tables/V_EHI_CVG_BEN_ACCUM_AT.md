...
| 2 | `BUCKET_ID` | NUMERIC | PK shared | This is the bucket ID this row corresponds to. |
| 3 | `BUCKET_ID_BUCKET_NAME` | VARCHAR |  | The name of the bucket. |
| 4 | `ROLL_PERIOD_KEY` | VARCHAR | PK | This column stores a unique key that is used to identify the data row. |
...
| 6 | `ROLL_PRD_END` | NUMERIC |  | Contains the roll period end date for the bucket. It is only populated for admission roll period buckets. |
...
| 11 | `BEN_AMOUNT` | NUMERIC |  | This is the total amount currently in the bucket for this row (i.e. this coverage, patient, bucket, and roll period combination). |
