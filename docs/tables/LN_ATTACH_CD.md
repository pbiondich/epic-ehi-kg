...
> This table holds codes representing the method used to transmit the attachment to the payor. The LN_ATTACH_TYP, LN_ATTACH_CD, and LN_ATTACH_DCN tables are related to one another. The number of lines in these tables will always be the same and values on the same line in each table will correspond to one another.
...
| 4 | `LN_ATTACH_CD` | VARCHAR |  | This item holds a code representing the method by which the attachment is transmitted to the payer. |
