# DOCS_RCVD_STG_SRC_ORGS

> Table to hold source organization for cancer stages with multiple sources.
...
| 6 | `STG_MULT_SRC_ORG_ID` | NUMERIC |  | Source organizations for cancer stages with multiple sources. |
| 7 | `STG_MULT_SRC_ORG_ID_EXTERNAL_NAME` | VARCHAR |  | Organization's external name used as the display name on forms and user interfaces. |
