# DOCS_RCVD_TXP_M_SRC_RM

> Related multi item for multiple source organisations for transplant data.
...
| 6 | `TXP_MULT_SRC_DXO_ID` | NUMERIC |  | Source organizations for transplant info with multiple sources. |
| 7 | `TXP_MULT_SRC_DXO_ID_EXTERNAL_NAME` | VARCHAR |  | Organization's external name used as the display name on forms and user interfaces. |
