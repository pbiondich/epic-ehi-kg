...
| 4 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple effective dates. |
| 5 | `CONTACT_DATE` | DATETIME |  | The date of the Filing Order History (FOH) contact in calendar format. |
| 6 | `EFF_DATE` | DATETIME |  | Stores the "effective date from" for the member on the coverage. I CVG 320 is stored if present, otherwise I CVG 400 is used. |
