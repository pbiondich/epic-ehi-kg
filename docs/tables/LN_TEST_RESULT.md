...
> This table holds the hematocrit, hemoglobin, or other test result associated with the service line. The LN_TEST_RESULT_CD, LN_TEST_RESULT_QUA, and LN_TEST_RESULT tables are related to one another. The number of lines in these tables will always be the same and values on the same line in each table will correspond to one another.
...
| 4 | `LN_TEST_RESULT` | NUMERIC |  | This item holds the hematocrit, hemoglobin, or other test result for the service line. |
