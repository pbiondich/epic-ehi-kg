# PAT_ENC_DSS_COMMENTS

> This table extracts comment for questions entered by user to receive Decision Support System (DSS) response.
...
| 3 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple comments for order specific questions associated with a patient order from the table PAT_ENC_DSS. |
...
| 7 | `DSS_COMMENT` | VARCHAR |  | The comment to a question entered by a user to receive a Decision Support System (DSS) response. |
