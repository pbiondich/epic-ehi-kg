# CRIT_CARE_CHG_ENTRY_USER

> This table stores the information for Critical Care Charging users on time entries. Time entries are duration-based incidents of critical care providers enter in Hyperspace.
...
| 6 | `ROW_USER_ID` | VARCHAR |  | Stores the lists of users for a patient's critical care charging time entry. |
| 7 | `ROW_USER_ID_NAME` | VARCHAR |  | The name of the user record. This name may be hidden. |
