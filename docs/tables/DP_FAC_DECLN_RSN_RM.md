...
> This table contains the reasons why a service declined a patient's request for continued care or services.
...
| 3 | `VALUE_LINE` | INTEGER | PK | The line number of one of the multiple selected services that are associated with the patient and the service providers from the DP_FACILITY table. |
...
| 6 | `DP_FAC_DECLN_RSN_C_NAME` | VARCHAR | org | Reason a service declines a patient request. |
