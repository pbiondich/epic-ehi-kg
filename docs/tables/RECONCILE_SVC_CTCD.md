...
> This table contains line-level category codes received through claims reconciliation. This information is only populated if the line-level category code was mapped to an Epic category value.
...
| 6 | `LN_CATEGORY_CODE_C_NAME` | VARCHAR | org | Line-level status category code. This value is only populated if the line-level status code matches a category abbreviation inside of the system. |
