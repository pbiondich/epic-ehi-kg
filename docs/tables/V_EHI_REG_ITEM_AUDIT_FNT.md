# V_EHI_REG_ITEM_AUDIT_FNT

> Financial assistance tracker audit events.This view contains audit information for financial assistance tracker records for the purpose of Electronic Health Information (EHI) export. If not in the EHI context, REG_ITEM_AUDIT_FNT and REG_ITEM_AUDIT_LINES_FNT should be used instead.
...
| 3 | `AUDIT_INSTANT_LOCAL_DTTM` | DATETIME (Attached) |  | The instant the audit was recored in the local time zone where the change took place. |
| 4 | `AUDIT_INSTANT_UTC_DTTM` | DATETIME (UTC) |  | The instant the audit was recorded in the UTC time zone. |
| 5 | `FIN_ASST_TRACKER_ID` | NUMERIC | FK→ | Financial assistance tracker that was changed in this audit event. |
