# ENC_METRIC_ENGINE_VAR

> Contains population specific variables that were used in calculating each set of metrics in this encounter.
...
| 7 | `FLO_MET_VARIABLE_C_NAME` | VARCHAR |  | This related group stores information about metrics that are calculated from the flowsheet metrics engine. The most recent entry in this group for a given data context will have corresponding data for each metric in I EPT 87750, and earlier entries will have corresponding data in the history items for flowsheet metrics, in I EPT 87760. This item stores the population variables of the corresponding metrics, with their corresponding values in I EPT 87737. |
